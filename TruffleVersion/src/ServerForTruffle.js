...
console.log('CompanyData[0].length: ' + CompanyData[0].length);
  ...
  res.render('DisplayShareholders',{varCompanies: CompanyNames,varTableRows:[]});
})
...
console.log('CompanyData[0].length: ' + CompanyData[0].length);
	...
	res.render('DisplayShareholders',{varCompanies: CompanyNames,varTableRows:tableRows});
})
...
app.post('/UploadVote', function (req, res) {
	...
	res.render('CreateVote3', {varInfoBar: "The new vote has been uploaded!"});
});
...
async function registerVote(Name,InvestorID,CompanyName,VoteID,VoteOptionSelectedID,VoteOptionSelectedText){
	...
	//run preliminary tests
		...
		let expiry = VDContract.getVoteExpiryDate(parseInt(VoteID));
		...
		//getVoterCount
		let voterCount = await VDContract.getVoterCount(parseInt(VoteID));
		console.log("voterCount: " + voterCount+ "|");		
		//checkOwner
		let contractOwner = await VDContract.getOwner();
...
console.log('CompanyData[0].length: ' + CompanyData[0].length);
	...
	res.render('VoteResults', {varQuestionNumber: 'Question', varCompanies: CompanyNames, varExpirationDate: "N/A", varQuestion: "Question: N/A", varOptionOne: "Option 1: ", varOptionOnePercentage: "N/A", varOptionTwo: "Option 2: ", varOptionTwoPercentage: "N/A", varPercentageOfVoters: "N/A", varNumOfVoters: 0, varTableRows: []});
})
...
app.get('/CreateVote1', async function (req, res) {
	...
	//for each company we need to get the 
  	...
  	res.render('CreateVote1', {varCompanies: CompanyData[0], dateIs: dateControlIs, timeIs: timeControlIs});
...
app.get('/CreateVote3', function (req, res) {
	...
	res.render('CreateVote3', {varInfoBar: ""});
})
...
async function setVotingDataOwner(votingDataAddress,votingLogicTransactionHash){
	...
	try{
		...
		var transactionHash = VDContract.setOwner(votingLogicAddress,{gas: 5000000}, function(error, result){
			if (!error) {
				console.log('Function succes:  ' + result);
			} else {
